...
# 登陆check 地址
LOGIN_URL = 'https://unite.nike.com/loginWithSetCookie?locale=zh_CN&backendEnvironment=default'
# 提交订单地址
...
class ShoeInfo(object):
    ...
    def __repr__(self):
        return '鞋子ID: %s \n 鞋子颜色: %s \n 鞋子地址:%s ' % (self.info_id, self.shoe_color, self.info_url)
...
class AddToCartTask(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            add_to_cart_url = PUT_ORDER_URL + '&_=' + str(int(1000 * time.time()))
            if DEBUG:
                pdb.set_trace()
            res = session.get(add_to_cart_url, params=self.param.__dict__, headers=HEADERS)
            ...
            if status_code == 200:
                ...
                if res_status_match:
                    ...
                    if res_status == 'success':  # 添加到购物车成功
                        logging.info('[第%s次] 恭喜添加购物车成功.......; 耗时%s秒', retry_times, (end - start))
                        r.publish('addToCart-notice-channel', 'SUCCESS')
                        break
                        ...
                        fail_message_match = fail_message_pattern.search(res.content)
                        if fail_message_match:
                            logging.info('添加购物车失败; 原因:%s', fail_message_match.group(1))
    ...
    if pd_id_match:
        ...
        if status_code == 200:
            ...
            if shoe_base_info_match:
                ...
                order_param = AddToCartParam(0, shoe_price, shoe_id, shoe_name, shoe_category)
                logging.info('鞋子基本信息如下:')
                logging.info('名称:%s', shoe_name)
                logging.info('价格:%s', shoe_price)
                logging.info('--------------')
                # 抠出正还在售的鞋子清单正则
                in_stock_list_pattern = re.compile('<ul\sdata-status="IN_STOCK".*>(?P<in_stock_list>[.|\s\S]*?)<\/ul>')
                in_stock_list_matcher = in_stock_list_pattern.search(content)
                if in_stock_list_matcher:
                    in_stock_list = in_stock_list_matcher.group('in_stock_list')
                    if in_stock_list:
                        ...
                        shoe_info_matcher = shoe_info_pattern.finditer(in_stock_list)
                        if shoe_info_matcher:
                            ...
                            if raw_pd_id != order_param.productId:
                                ...
                                # 匹配出鞋子的基本信息; 名称以及43码对应的货号
                                common_sku_pattern = re.compile('"displaySize":"43","sku":"(?P<id>\d{8})"')
                                common_sku_match = common_sku_pattern.search(new_res.content)
                                if common_sku_match:
                                    order_param.skuId = common_sku_match.group('id')
